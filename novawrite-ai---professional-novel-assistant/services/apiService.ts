...
export const novelApi = {
  ...
  // 生成当前章节内容（后端任务）
  writeChapter: async (novelId: string, volumeId: string, chapterId: string): Promise<{ task_id: string }> => {
    return apiRequest<{ task_id: string }>(`/api/novels/${novelId}/volumes/${volumeId}/chapters/${chapterId}/write-chapter`, {
      method: 'POST',
    });
  },
