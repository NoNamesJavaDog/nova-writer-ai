...
class GeminiProvider(AIServiceProvider):
    ...
    基于 Google Gemini API 实现的 AI 服务提供商
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            proxy: 代理地址（可选，支持 HTTP/HTTPS/SOCKS5）
            model: 模型名称
            timeout_ms: 超时时间（毫秒）
            **kwargs: 其他参数
        """
        ...
        self.client = None
        super().__init__(api_key, proxy, **kwargs)

    def _configure_client(self):
        """配置 Gemini 客户端

        设置代理、超时等配置，初始化 genai.Client
        """
        # 配置代理（如果设置了）
...
"""
    ...
    def _handle_geo_restriction_error(self, e: Exception, operation: str):
        """处理地理位置限制错误
        ...
        if "location is not supported" in error_msg or "FAILED_PRECONDITION" in error_msg:
            ...
            raise Exception(f"{operation}失败: {friendly_msg}")
        raise Exception(f"{operation}失败: {error_msg}")
...
"""
            ...
            full_outline = self._ensure_text_length_range(
                full_outline,
                min_chars=6000,
                max_chars=10000,
                title=title,
...
- "summary"（卷的简要描述，50-100字）"""
            ...
            volumes_response = self.client.models.generate_content(
                model=self.model,
            ...
            self._handle_geo_restriction_error(e, "生成大纲")
...
"""
            ...
            self._handle_geo_restriction_error(e, "生成卷大纲")
...
仅返回 JSON 数组，每个对象包含以下键："title"（标题，不要带章节编号）、"summary"（摘要）、"aiPromptHints"（AI提示）。"""
    ...
    async def modify_outline_by_dialogue(
        ...
        try:
            ...
            # 从 dialogue_data 中提取所有需要的字段
            title = dialogue_data.get("title", "")
            genre = dialogue_data.get("genre", "")
            synopsis = dialogue_data.get("synopsis", "")
            characters = dialogue_data.get("characters", [])
            world_settings = dialogue_data.get("world_settings", [])
            timeline = dialogue_data.get("timeline", [])
...
当前完整大纲：
{outline[:3000]}{'...' if len(outline) > 3000 else ''}
...
⚠️ 注意：上述前文是相关章节，请认真阅读并确保本章内容完全不同。
"""
...
现在请开始创作，仅输出章节正文内容（不要输出标题）："""
            ...
            self._handle_geo_restriction_error(e, "生成章节内容")
...
"""
            ...
            logger.warning(f"章节摘要生成失败，使用截断: {str(e)}")
...
"""
    ...
    async def generate_foreshadowings_from_outline(
        ...
        try:
            ...
            prompt = f"""基于以下小说大纲，生成伏笔列表（5-15个伏笔）：
大纲：{full_outline[:2000]}
...
仅返回钩子文本内容（不要包含"钩子"、"悬念"等标签词），直接输出文本："""
            ...
            logger.warning(f"提取下一章钩子失败: {str(e)}")
            return ""
